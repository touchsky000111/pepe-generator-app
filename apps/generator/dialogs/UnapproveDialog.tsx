...
export const UnapproveDialog: FC<Props> = ({ onClose }) => {
  ...
  if (isUpdating) {
    return (
      <Dialog open>
        <DialogTitle>
          Unapproving {selectedIds.length} Pepe{selectedIds.length === 1 ? '' : 's'}...
        </DialogTitle>
        <DialogContent>
  ...
  return (
    <Dialog open onClose={onClose}>
      ...
      <DialogContent>
        {selectedIds.length} Pepe{selectedIds.length === 1 ? '' : 's'} will be unapproved.
      </DialogContent>
      <DialogActions>
