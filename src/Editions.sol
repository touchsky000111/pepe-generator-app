...
contract Editions is ERC1155, ERC1155Burnable, ERC1155Supply, AccessControl {
  ...
  function _beforeTokenTransfer(
    ...
    super._beforeTokenTransfer(__operator, __from, __to, __ids, __amounts, __data);

    if (__from == address(0) && __ids.length > 1) {
      for (uint256 i = 0; i < __ids.length; i++) {
        if (_editions[__ids[i]].maxSupply != 0) {
          if (totalSupply(__ids[i]) > _editions[__ids[i]].maxSupply) revert NotEnoughSupply();
        }
      }
    }
  }
  ...
  function releaseEdition(
    ...
    _editions[editionId] = Edition({frozenMetadata: false, maxSupply: __maxSupply, uri: __uri});
  ...
  function mint(
    ...
    if (_editions[__id].maxSupply != 0) {
      if (totalSupply(__id) + __amount > _editions[__id].maxSupply) revert NotEnoughSupply();
    }
  ...
  function mintMany(
    ...
    if (_editions[__id].maxSupply != 0) {
      ...
      if (totalSupply(__id) + totalAmount > _editions[__id].maxSupply) revert NotEnoughSupply();
    }
   ...
   */
  function maxSupply(uint256 __id) external view onlyExistingEdition(__id) returns (uint256) {
  ...
  function uri(
    uint256 __id
  ) public view virtual override onlyExistingEdition(__id) returns (string memory) {
